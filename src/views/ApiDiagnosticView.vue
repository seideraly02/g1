...
<template>
  <section class="screen-page mx-auto min-h-screen max-w-[760px] bg-[#f8fafc] px-4 py-6">
    <header class="flex items-center justify-between gap-4">
      <button
        class="min-h-11 text-sm font-bold text-[#2468f2]"
        type="button"
        @click="router.back()"
      >
        Артқа
      ...
      <div class="min-w-0 flex-1">
        ...
        <div class="mt-2 h-2 overflow-hidden rounded-full bg-[#e3e8f0]">
          <div
            class="h-full rounded-full bg-[#2468f2] transition-all"
    ...
    <main class="mt-8">
      ...
      <div
        v-else-if="error"
        class="rounded-2xl bg-[#fff0f1] p-4 text-sm text-[#b4232a]"
        role="alert"
      >
        {{ error }}
      </div>
      ...
      <div v-else-if="result" class="rounded-3xl bg-white p-6 shadow-sm">
        ...
        <h2 class="mt-2 text-3xl font-black text-[#111b34]">
          {{ result.correct }} / {{ result.total }}
        </h2>
        <p class="mt-3 text-sm leading-6 text-[#536178]">
          Бұл бес сұрақтық бастапқы нәтиже. Ол нақты ҰБТ болжамы емес, тек оқу жоспарын бастауға
          арналған.
        </p>
        <button
          class="mt-6 min-h-12 w-full rounded-2xl bg-[#2468f2] px-4 font-black text-white"
          type="button"
      ...
      <template v-else-if="currentQuestion">
        <span class="rounded-lg bg-[#dfecff] px-2 py-1 text-xs font-extrabold text-[#2468f2]">{{
        ...
        <div class="mt-5 space-y-3">
          <button
            ...
            :class="
              selectedIndex === index
                ? 'border-[#2468f2] ring-2 ring-[#b9d2ff]'
                : 'border-[#dfe5ee]'
            "
